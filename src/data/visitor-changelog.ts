...
export const visitorChangelog: ChangelogVersion[] = [
  {
    ...
    fixed: [
      '广场「显示设置」里的时间排序、评论筛选、评论排序、一行几个，展开后选项可以正常点选。',
      '渐变风格日间模式：广场工具栏和下方面板的按钮、状态 Tab 不再是白字叠白底；选中态改成浅紫渐变底配黑字。',
      '渐变风格日间模式：广场分类筛选、上传页「单篇 / 批量」等同配色选中按钮，字体改成黑色。',
      '渐变风格日间模式：顶部「续写 / 新增日历 / 广场 / 上传」改成磨砂白底深紫字，选中用主题紫，不再白字叠浅底。',
      '粗野主义夜间模式：广场「全部 / 未标记 / 收藏 / 不喜欢」选中后能看见字。',
      '粗野主义夜间模式：广场工具栏的折叠、随机、导出、更多、搜索与「筛选」一样改成黑字，不再残留白图标。',
      '新拟态日间模式：广场「显示设置」里的下拉框改成拟态灰底，不再是突兀的白底。',
    ],
  },
  {
    version: '1.6',
    added: [
      ...
      '「主题」面板里的 ⚙ 高级设置：可单独调节字号、UI 缩放、背景模糊 / 不透明度 / 暗化层；字体源可选用系统字体、主题默认字体，或粘贴字体链接；可锁定背景滑块防误触。',
    ...
    improved: [
      ...
      '上传小剧场时，分类输入框会按已有分类实时搜索，并把下方分类按首字母排序。',
    ],
    fixed: [
      '游客提交的修改通过后会覆盖旧版并只保留新版；后台再删旧版时，已通过的新版不会一起消失。',
