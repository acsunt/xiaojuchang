...
export function PlazaCalendarPanel({ plays, onOpenPlay }: PlazaCalendarPanelProps) {
  ...
  const dayBucketMap = useMemo(() => new Map(dayBuckets.map((item) => [item.dayKey, item])), [dayBuckets]);
  const availableMonthKeys = useMemo(
    () => [...new Set(dayBuckets.map((item) => item.monthKey))].sort((left, right) => left.localeCompare(right)),
    [dayBuckets],
  );
  ...
  useEffect(() => {
    ...
    setActiveMonthKey((current) => (availableMonthKeys.includes(current) ? current : latestMonthKey));
  ...
  const calendarCells = useMemo(() => buildCalendarCells(activeMonthKey, dayBucketMap), [activeMonthKey, dayBucketMap]);
  ...
  const selectYear = (year: string) => {
    ...
    const targetMonth = candidateMonths.find((item) => item.slice(5, 7) === activeMonthNumber) ?? candidateMonths[candidateMonths.length - 1];
    if (targetMonth) {
  ...
  return (
    <section className="form-panel plaza-calendar-panel">
      ...
      <p className="sub-copy plaza-calendar-intro">按当前筛选范围统计每天新增的小剧场，点日期即可查看当天列表。</p>
      ...
      <div className="plaza-calendar-picker-bar" ref={pickerAreaRef}>
              ...
              <div className="plaza-calendar-picker-menu" role="listbox" aria-label="选择年份">
                {availableYears.map((year) => (
                  <button
                    ...
                    className={year === activeYear ? 'plaza-calendar-picker-option active' : 'plaza-calendar-picker-option'}
                    key={year}
          ...
          <div className={monthPickerOpen ? 'plaza-calendar-picker open' : 'plaza-calendar-picker'}>
            ...
            {monthPickerOpen ? (
              <div className="plaza-calendar-picker-menu plaza-calendar-picker-menu-months" role="listbox" aria-label="选择月份">
                {availableMonthsOfYear.map((monthKey) => (
                  <button
                    aria-selected={monthKey === activeMonthKey}
                    className={monthKey === activeMonthKey ? 'plaza-calendar-picker-option active' : 'plaza-calendar-picker-option'}
                    key={monthKey}
        ...
        {calendarCells.map((cell) => {
          ...
          return (
            ...
            >
              ...
              <span className="plaza-calendar-day-count">{cell.count > 0 ? `${cell.count} 篇` : '—'}</span>
            </button>
          );
        })}
      </div>
